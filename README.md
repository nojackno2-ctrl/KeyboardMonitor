...
2. 從右上角的下拉選單切換 100%／80% TKL／60% 三種鍵盤配置，選擇與您手邊鍵盤相符的佈局。
3. 依序按過鍵盤上每一個按鍵，觀察是否有按鍵沒有亮起（可能失靈）或持續亮紅（可能卡鍵）。
4. 同時按住多顆按鍵，可用來確認鍵盤的全鍵無衝突（NKRO）表現。
5. 在「打字與延遲測試 (TYPING & LATENCY TEST)」文字框中輸入文字，即可看到 WPM 與 KPS 數值；按 <kbd>Esc</kbd> 可快速清空該文字框。
6. 點擊滑鼠檢測區可驗證左鍵、右鍵、中鍵、側鍵（X1／X2）與滾輪上下捲動。
7. 按右上角的「清除重設」按鈕，即可清空所有按鍵狀態、統計數值與事件日誌，重新開始一輪測試。
...
<br>
點擊右上角的「清除重設」按鈕，即可將所有按鍵恢復至初始狀態，並清空日誌與 KPS 統計數據。
</details>
